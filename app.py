...
MODEL_FILES = {
    ...
    'svm_w2v_model.pkl': 'svm_w2v_model.pkl',
    'w2v_model.bin': 'w2v_model.bin',
...
st.markdown("Enter a social media post below. The system will predict whether it shows ADHD indicators and explain which words influenced the prediction.")
st.divider()
...
show_lime = st.checkbox("Show LIME explanation (top 10 influencing words)", value=True)
    ...
    if not text_input.strip():
        ...
        with st.spinner("Loading models and predicting..."):
            ...
            if model_choice == "SVM + TF-IDF":
                ...
                vec = np.mean([w2v_model.wv[w] for w in clean.split() if w in w2v_model.wv] or [np.zeros(100)], axis=0)
                ...
                def predict_fn(texts):
                    ...
                    for t in texts:
                        ...
                        vecs.append(v)
                    decisions = svm_w2v.decision_function(vecs).reshape(-1, 1)
        ...
        if show_lime:
            ...
            st.caption("Positive scores push toward ADHD. Negative scores push toward Non-ADHD.")

            for word, score in explanation:
                bar_color = "🟠" if score > 0 else "🔵"
                st.write(f"{bar_color} **{word}**: {score:.4f}")

st.divider()
st.markdown("""
**About this system**
- Dataset: 36,782 balanced Reddit posts (r/ADHD + Normal class)
- Models: SVM+TF-IDF (96.56%) | SVM+Word2Vec (88.67%) | LSTM+Word2Vec (96.94%) | BERT (98.11%)
